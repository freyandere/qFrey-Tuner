...
class NetworkTester:
    ...
    SERVERS = [
        {"name": "Cloudflare (Global Edge)", "url": "https://speed.cloudflare.com/__down", "up_url": "https://speed.cloudflare.com/__up"},
        {"name": "Microsoft Azure (Europe)", "url": "https://azspeedtest.blob.core.windows.net/speedtest/100MB.bin", "up_url": ""},
    ...
    def get_best_server() -> dict:
        ...
        for server in NetworkTester.SERVERS:
            try:
                start = time.time()
                requests.head(server["url"], timeout=2)
    ...
    def _download_chunk(url: str, size: int) -> int:
        ...
        try:
            # Для Cloudflare добавляем параметр размера
            test_url = f"{url}?bytes={size}" if "__down" in url else url
    ...
    def test_download_speed_mbps() -> Tuple[float, str]:
        """Оценить скорость загрузки с использованием многопоточности."""
        server = NetworkTester.get_best_server()
        num_threads = 8  # Для 1Gbps+
        chunk_size = 25 * 1024 * 1024  # 25MB на поток
    ...
    @staticmethod
    def _upload_chunk(url: str, data: b"") -> int:
    ...
    def test_upload_speed_mbps() -> float:
        ...
        server = NetworkTester.SERVERS[0] # Cloudflare поддерживает __up
